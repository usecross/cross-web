...
@pytest.mark.asyncio
async def test_litestar_adapter_get_body():
...
@pytest.mark.asyncio
async def test_litestar_adapter_get_form_data():
...
@pytest.mark.asyncio
async def test_litestar_adapter_content_type_with_params():
    """Test content_type with parameters"""
    mock_request = Mock()
    mock_request.content_type = ("multipart/form-data", {"boundary": "----WebKitFormBoundary", "charset": "utf-8"})
    
    adapter = LitestarRequestAdapter(mock_request)
    
    content_type = adapter.content_type
    assert content_type == "multipart/form-data; boundary=----WebKitFormBoundary; charset=utf-8"


@pytest.mark.asyncio
async def test_litestar_adapter_content_type_no_params():
...
@pytest.mark.asyncio
async def test_litestar_adapter_empty_form():
